...
## &sect; Corrected kinematics value description
 ...
 > <sup>1</sup> For example, select your real robot type as the input nominal model form. If your TM robot is TM12S, then the user can type tm12s as the <urdf_from>.<br/>
...
> cd <workspace>
> source ./install/setup.bash
> cd tm2_ros2/tm_mod_urdf/tm_mod_urdf
...
> :bookmark_tabs: Note: In this chapter, we use the default project folder, "``tm2_ros2``", directly as the source directory. If you rename this folder or use a different directory name, executing the above command will trigger the following error: "``[Error] [modify_xacro]: workspace directory not found ``" on the screen.<br/>
> 
...
> ```bash
> cd tm2_ros2\tm_description\cobot_s\tm12s_description\xacro\
...
>   <xacro:include filename="$(find tm12s_description)/xacro/macro.tm12s-nominal.urdf.xacro" />
> # After modification : (Replace with your actual newly generated Xacro file)
>   <xacro:include filename="$(find tm12s_description)/xacro/user_defined.urdf.xacro" />
> ```
...
>
> [!NOTE]
> For TM12S* Users: If your real Robot is a `TM12S*`, set the <urdf_from> parameter to lowercase `tm12s*` and to customize the robot description, modify the `tm12s*.urdf.xacro` file is also located inside the __tm12s_description__ folder (shared by the <sup>4</sup> __TM12S series__).<br/>
>> :bookmark_tabs: Note:``tm_description``: Package Branch Structure: TM AI Cobot S Series reference
...
>>                        ├── rviz/
>>                        ├── xacro/
>>                        │   ├──  macro.tm12s-nominal.urdf.xacro
>>                        │   ├── tm12s.urdf.xacro
>>                        │   ├──  macro.tm12s*-nominal.urdf.xacro
>>                        │   ├──  tm12s*.urdf.xacro
>>                        │   ├──  macro.materials.xacro
>>                        │   └──  macro.xxxx*.xacro
>>                        ├── CMakeLists.txt
>>                        └── package.xml
>> ```
