...
size_t _recv_fake_svr_pack_data(char *buf)
{
  ...
  TmSvrData::build_TmSvrData(svr_pack.data, "0", TmSvrData::Mode::BINARY,
      svr_pack.content.data(), svr_pack.content.size(), TmSvrData::ScrType::Shallow);
...
int TmCommunication::connect_with_timeout(int sockfd, const char *ip, unsigned short port, int timeout_ms)
{
  ...
  print_once("TM_COM: robot_ip:=%s", ip);
  ...
  else
  {
    ...
    if (rv == 0) {
      if (_remote_ctrl) {
        print_warn("TM_COM: %hu remote control connection timeout count=%d", (unsigned short)port, (int)timeoutcount);
      }
      else
      {
        print_warn("TM_COM: %hu connection timeout count=%d", (unsigned short)port, (int)timeoutcount);
      }
      // errno = ETIMEDOUT;
      return -1;
    }
...
bool TmCommunication::connect_socket(std::string errorName, int timeout_ms, bool remote_ctrl)
{
  _isConnected = false;
  ...
  if (remote_ctrl && _port == 5890) {
    _remote_ctrl = remote_ctrl;
    _port = 5892;  // Remote control mode.
    print_info("TM_COM: Connect to remote control mode");
  }
...
TmCommRC TmCommunication::recv_spin_once(int timeval_ms, int *n)
{
  ...
  if (loop_cnt == 10 || pack_cnt == 10) {
    print_warn("High Latency Warning: Over 10 packets queued. Please check network stability!");
  }
  if (pack_cnt == 0) {
