  ...
  while (rclcpp::ok()) {
    ...
    while (rclcpp::ok() && iface_.svr.is_connected() && !reconnect) {
      ...
      switch (rc) {
        ...
        case TmCommRC::ERR:
          print_info("TM_ROS2: (TM_SVR) rc=%d", static_cast<int>(rc));
          reconnect = true;
          break;
        default:
          break;
...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1) {
    ...
    if (host.find("robot_ip:=") != std::string::npos) {
      ...
      std::cout << "Only the keywords 'ip' or 'robot_ip' are supported, while you entered '" << host << "' in argv[1]" << std::endl;
    }
  }
  else
  {
    print_fatal("Incorrect argument, unable to connect to TM Cobot.");
    rclcpp::shutdown();
    return 1;
  }
