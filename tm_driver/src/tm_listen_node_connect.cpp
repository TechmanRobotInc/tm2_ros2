...
ListenNodeConnection::ListenNodeConnection(TmDriver &iface,std::function<void(TmSctData)> sct_msg,
  std::function<void(std::string, std::string)> sta_msg, bool is_fake_, bool is_rc_)
  : iface(iface), sct_(iface.sct), sct_msg(sct_msg), sta_msg(sta_msg)
{
  isRun = true;
  sct_.set_remote_connect(is_rc_);  // If true, switch to remote control settings mode.
  if (!is_fake_) {
  ...
  for (auto &pack : pack_vec) {
    switch (pack.type) {
      ...
      case TmPacket::Header::TMSCT:
        ...
        TmSctData::build_TmSctData(sct.sct_data, pack.data.data(), pack.data.size(), TmSctData::ScrType::Shallow);
      ...
      case TmPacket::Header::TMSTA:
        ...
        TmStaData::build_TmStaData(sct.sta_data, pack.data.data(), pack.data.size(), TmStaData::ScrType::Shallow);
