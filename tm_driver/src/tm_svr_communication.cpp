...
TmSvrCommunication::TmSvrCommunication(const std::string &ip, int recv_buf_len, std::condition_variable *cv)
...
bool TmSvrCommunication::start_tm_svr(int timeout_ms)
{
  ...
  bool rb = connect_socket("Ethernet slave communication", timeout_ms, 0);
...
TmCommRC TmSvrCommunication::send_content(const std::string &id, TmSvrData::Mode mode, const std::string &content)
{
  ...
  TmSvrData cmd{id, mode, cntt.data(), cntt.size(), TmSvrData::ScrType::Shallow};
  TmPacket pack{cmd};
...
{
  std::string cntt = content;
  TmSvrData cmd{id, TmSvrData::Mode::STRING, cntt.data(), cntt.size(), TmSvrData::ScrType::Shallow};
  TmPacket pack{cmd};
  return send_packet_all(pack);
}
    ...
    while (_keep_thread_alive && is_connected() && !reconnect) {
      ...
      switch (rc) {
        ...
        case TmCommRC::TIMEOUT:
          print_info("Ethernet slave communication: rc=%d", static_cast<int>(rc));
          reconnect = true;
          break;
        default:
          break;
...
void TmSvrCommunication::reconnect_function()
{
  ...
  if (_keep_thread_alive && _reconnect_timeval_ms >= 0) {
    ...
    connect_socket("Ethernet slave re-connection", _reconnect_timeout_ms, 0);
  }
}
...
TmCommRC TmSvrCommunication::tmsvr_function()
{
  ...
  for (auto &pack : pack_vec) {
    if (pack.type == TmPacket::Header::CPERR) {
      ...
      TmSvrData::build_TmSvrData(data, pack.data.data(), pack.data.size(), TmSvrData::ScrType::Shallow);
