...
TmSctRos2::TmSctRos2(rclcpp::Node::SharedPtr node, TmDriver &iface, bool is_fake, bool is_rc)
  : node(node)
  , iface_(iface)
  , is_fake_(is_fake)
  , is_rc_(is_rc)
{
  ...
  if (!is_fake_) {
    listenNodeConnection = std::make_unique<ListenNodeConnection>(
        iface_, std::bind(&TmSctRos2::sct_msg, this, std::placeholders::_1),
        std::bind(&TmSctRos2::sta_msg, this, std::placeholders::_1, std::placeholders::_2), is_fake_, is_rc_);
  ...
  connect_tm_srv_ = node->create_service<tm_msgs::srv::ConnectTM>(
    "connect_tmsct", std::bind(&TmSctRos2::connect_tmsct, this, std::placeholders::_1, std::placeholders::_2));

  send_script_srv_ = node->create_service<tm_msgs::srv::SendScript>(
    "send_script", std::bind(&TmSctRos2::send_script, this, std::placeholders::_1, std::placeholders::_2));

  set_event_srv_ = node->create_service<tm_msgs::srv::SetEvent>(
    "set_event", std::bind(&TmSctRos2::set_event, this, std::placeholders::_1, std::placeholders::_2));

  set_io_srv_ = node->create_service<tm_msgs::srv::SetIO>(
    "set_io", std::bind(&TmSctRos2::set_io, this, std::placeholders::_1, std::placeholders::_2));

  set_positions_srv_ = node->create_service<tm_msgs::srv::SetPositions>(
    "set_positions", std::bind(&TmSctRos2::set_positions, this, std::placeholders::_1, std::placeholders::_2));

  ask_sta_srv_ = node->create_service<tm_msgs::srv::AskSta>(
    "ask_sta", std::bind(&TmSctRos2::ask_sta, this, std::placeholders::_1, std::placeholders::_2));
}

TmSctRos2::~TmSctRos2() {
  print_info("TM_ROS: ~TmSctRos2");
