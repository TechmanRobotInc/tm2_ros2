...
void TmPacket::setup_header(Header type)
{
  ...
  switch (type) {
    ...
    case Header::TMSVR:
      header = HDR_TMSVR;
      break;
    case Header::PACKAGE_INCOMPLETE:
      header = PACKAGE_INCOMPLETE;
      break;
    case Header::OTHER:
      break;
  }
...
TmSvrData::ErrorCode TmSvrData::_error_code(const char *buf)
{
  ...
  int ic = std::atoi(cc);
  if (ic < (int)(ErrorCode::SettingIncomplete)) {
    return ErrorCode(ic);
  }
  else
  {
    return ErrorCode::SettingIncomplete;
  }
}
...
void TmSvrData::build_TmSvrData(TmSvrData &data, const TmSvrData &other, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
...
void TmSvrData::build_TmSvrData(TmSvrData &data, const std::string &id, Mode mode, const char *content, size_t len, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
...
void TmSvrData::build_TmSvrData(TmSvrData &data, const char *bytes, size_t size, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
  data._is_valid = false;
  ...
  if (cmode > char(Mode::UNKNOWN) || bytes[ind_e] < char(Mode::RESPONSE)) {
    return;
  }
...
void TmSctData::build_TmSctData(TmSctData &data, const TmSctData &other, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
...
void TmSctData::build_TmSctData(TmSctData &data, const std::string &id, const char *script, size_t len, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
  data._is_valid = false;
  data._script_id = id;
...
void TmSctData::build_TmSctData(TmSctData &data, const char *bytes, size_t size, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
  data._is_valid = false;
...
void TmStaData::build_TmStaData(TmStaData &data, const TmStaData &other, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
...
void TmStaData::build_TmStaData(TmStaData &data, const std::string &sub_cmd, const char *sub_data, size_t len, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
  data._is_valid = false;
...
void TmStaData::build_TmStaData(TmStaData &data, unsigned char sub_cmd, const char *sub_data, size_t len, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
  data._is_valid = false;
...
void TmStaData::build_TmStaData(TmStaData &data, const char *bytes, size_t size, ScrType type)
{
  data._is_copy = (type != ScrType::Shallow);
  data._is_valid = false;
