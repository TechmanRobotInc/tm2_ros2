...
bool TmSctCommunication::start_tm_sct(int timeout_ms)
{
  ...
  bool rb = connect_socket("listen node communication", timeout_ms, _is_RemoteCtrl);
...
void TmSctCommunication::set_remote_connect(bool is_rc)
{
  _is_RemoteCtrl = is_rc;  // If TMflow remote control configuration is set correctly, enable this feature to take effect.

  if (_is_RemoteCtrl) {
    // Use the TMscript command interface to control the robot remotely. During the remote operation, ...like the Listen Node.
    print_info("Set to remote control mode communication");
  }
}
...
TmCommRC TmSctCommunication::send_script_str(const std::string &id, const std::string &script)
{
  ...
  TmSctData cmd{id, sct.data(), sct.size(), TmSctData::ScrType::Shallow};
  TmPacket pack{cmd};
...
TmCommRC TmSctCommunication::send_sta_request(const std::string &subcmd, const std::string &subdata)
{
  ...
  TmStaData req{subcmd, data.data(), data.size(), TmStaData::ScrType::Shallow};
  TmPacket pack{req};
  return send_packet_all(pack);
}
    ...
    while (_keep_thread_alive && is_connected() && !reconnect) {
      ...
      switch (rc) {
        ...
        case TmCommRC::NOTCONNECT:
          print_info("Listen node communication: rc=%d", static_cast<int>(rc));
          reconnect = true;
          break;
        default:
          break;
...
void TmSctCommunication::reconnect_function()
{
  ...
  if (_keep_thread_alive && _reconnect_timeval_ms >= 0) {
    ...
    connect_socket("Listen node communication", _reconnect_timeout_ms, _is_RemoteCtrl);
  }
}
  ...
  for (auto &pack : pack_vec) {
    switch (pack.type) {
      ...
      case TmPacket::Header::TMSCT:
        ...
        /* TmSctData::build_TmSctData(sct_data, pack.data.data(), pack.data.size(), TmSctData::ScrType::Shallow);
        {
          std::lock_guard<std::mutex> lck(mtx_sct);
        ...
        TmSctData::build_TmSctData(sct_data_tmp, pack.data.data(), pack.data.size(), TmSctData::ScrType::Shallow);
        {
          std::lock_guard<std::mutex> lck(mtx_sct);
          TmSctData::build_TmSctData(sct_data, sct_data_tmp, TmSctData::ScrType::Deep);
        }
      ...
      case TmPacket::Header::TMSTA:
        ...
        TmStaData::build_TmStaData(sta_data_tmp, pack.data.data(), pack.data.size(), TmStaData::ScrType::Shallow);
        {
          std::lock_guard<std::mutex> lck(mtx_sta);
          TmStaData::build_TmStaData(sta_data, sta_data_tmp, TmStaData::ScrType::Deep);
        }
