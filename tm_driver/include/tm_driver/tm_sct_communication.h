...
public:
  explicit TmSctCommunication(const std::string &ip, int recv_buf_len, bool &isOnListenNode, std::condition_variable *cv = nullptr);
  ~TmSctCommunication();

  bool start_tm_sct(int timeout_ms = 0);
  void set_remote_connect(bool is_rc = 0);
  void halt();
