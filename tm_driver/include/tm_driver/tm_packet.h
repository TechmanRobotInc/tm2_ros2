...
public:
  ...
  explicit TmPacket(Header type, const std::vector<char> &data) : data(data)
  {
    setup_header(type);
  }

  explicit TmPacket(Header type, const std::string &data) : data{ std::begin(data), std::end(data) }
  {
    setup_header(type);
  }

  explicit TmPacket(const std::string &hdr, const std::vector<char> &data) : data(data)
  {
    setup_header(hdr);
  }

  explicit TmPacket(const std::string &hdr, const std::string &data) : data{ std::begin(data), std::end(data) }
  {
    setup_header(hdr);
  }
...
public:
  ...
  explicit TmSvrData(const TmSvrData &other, ScrType type)
  {
    build_TmSvrData(*this, other, type);
  }

  explicit TmSvrData(const std::string &id, Mode mode, const char *content, size_t len, ScrType type)
  {
    build_TmSvrData(*this, id, mode, content, len, type);
  }

  explicit TmSvrData(const char *data, size_t size, ScrType type)
  {
...
public:
  ...
  static void build_TmSvrData(TmSvrData &data, const TmSvrData &other, ScrType type);
  static void build_TmSvrData(TmSvrData &data, const std::string &id, Mode mode, const char *content, size_t len, ScrType type);
  static void build_TmSvrData(TmSvrData &data, const char *bytes, size_t size, ScrType type);
...
public:
  ...
  explicit TmSctData(const TmSctData &other, ScrType type)
  {
    build_TmSctData(*this, other, type);
  }

  explicit TmSctData(const std::string &id, const char *script, size_t len, ScrType type)
  {
    build_TmSctData(*this, id, script, len, type);
  }

  explicit TmSctData(const char *data, size_t size, ScrType type)
  {
...
public:
  ...
  static void build_TmSctData(TmSctData &data, const TmSctData &other, ScrType type);
  static void build_TmSctData(TmSctData &data, const std::string &id, const char *script, size_t len, ScrType type);
  static void build_TmSctData(TmSctData &data, const char *bytes, size_t size, ScrType type);
...
public:
  ...
  explicit TmStaData(const TmStaData &other, ScrType type)
  {
    build_TmStaData(*this, other, type);
  }

  explicit TmStaData(const std::string &sub_cmd, const char *sub_data, size_t len, ScrType type)
  {
    build_TmStaData(*this, sub_cmd, sub_data, len, type);
  }

  explicit TmStaData(const unsigned char sub_cmd, const char *sub_data, size_t len, ScrType type)
  {
    build_TmStaData(*this, sub_cmd, sub_data, len, type);
  }

  explicit TmStaData(const char *data, size_t size, ScrType type)
  {
...
public:
  ...
  static void build_TmStaData(TmStaData &data, const TmStaData &other, ScrType type);
  static void build_TmStaData(TmStaData &data, const std::string &sub_cmd, const char *sub_data, size_t len, ScrType type);
  static void build_TmStaData(TmStaData &data, unsigned char sub_cmd, const char *sub_data, size_t len, ScrType type);
  static void build_TmStaData(TmStaData &data, const char *bytes, size_t size, ScrType type);
