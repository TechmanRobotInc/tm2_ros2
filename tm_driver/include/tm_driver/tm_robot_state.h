...
public:
  ...
  std::vector<unsigned char> ctrller_DO() {
    ...
    ctrllerDO.assign(16, 0);
    for (int i = 0; i < 16; ++i) { ctrllerDO[i] = tmRobotStateDataToPublish.ctrller_DO[i]; }
    return ctrllerDO;
  }
  std::vector<unsigned char> ctrller_DI() {
    std::vector<unsigned char> ctrllerDI;
    ctrllerDI.assign(16, 0);
    for (int i = 0; i < 16; ++i) { ctrllerDI[i] = tmRobotStateDataToPublish.ctrller_DI[i]; }
    return ctrllerDI;
  }
