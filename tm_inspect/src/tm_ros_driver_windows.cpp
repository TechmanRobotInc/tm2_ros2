...
void MainWindow::initial_ui_component() {
  connect(ui->change_control_box_io0_button, SIGNAL(clicked()), this, SLOT(click_change_control_box_io0_button()));
  connect(ui->change_control_box_io15_button, SIGNAL(clicked()), this, SLOT(click_change_control_box_io15_button()));
...
void MainWindow::initial_status_ctrl_label() {
  ...
  set_text_nan_initial(ui->ctrl_do15_status_label);
...
void MainWindow::send_ui_feed_back_status(tm_msgs::msg::FeedbackState::SharedPtr msg) {
  ...
  if (msg->is_svr_connected)
  {
    ...
    if (msg->cb_digital_output.size() > 0) {
      ...
      if(msg->cb_digital_output[15] == 0) {
        set_text_high_low(false, ui->ctrl_do15_status_label, true);
      }
      else
      {
        set_text_high_low(true, ui->ctrl_do15_status_label, true);
      }
    }
  }
  else
...
void MainWindow::click_change_control_box_io0_button() {
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "Click [DO0 Ctrl] button");
  change_control_box_io0_button();
}

void MainWindow::click_change_control_box_io15_button() {
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "Click [DO15 Ctrl] button");
  change_control_box_io15_button();
}

void MainWindow::quit()
{
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "Click [Quit_GUI] button");
  QApplication::quit();
}

void MainWindow::initial_ui_page_to_ros_thread() {
  connect(this, SIGNAL(change_control_box_io0_button()), rosPage.get(), SLOT(change_control_box_io0_button()));
  connect(this, SIGNAL(change_control_box_io15_button()), rosPage.get(), SLOT(change_control_box_io15_button()));
}
