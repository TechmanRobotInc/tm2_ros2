...
void RosPage::svr_response_callback(tm_msgs::msg::SvrResponse::SharedPtr msg) {
  std::string re = current_time() + " [svr]-> id:" + msg->id + ", mode:" + std::to_string(msg->mode) + ", content->" +
                   msg->content + ", error_code->" + std::to_string(msg->error_code);
  send_to_ui_list(re);
}
...
void RosPage::send_service(rclcpp::Client<tm_msgs::srv::ConnectTM>::SharedPtr client, std::shared_ptr<tm_msgs::srv::ConnectTM::Request> request) {
  while (!client->wait_for_service(1s)) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("tm_inspect"), "Interrupted while waiting for the service. Exiting.");
      return;
    }
    RCLCPP_INFO_STREAM(rclcpp::get_logger("tm_inspect"), "service not available");
    return;
  }
  ...
  if (result.get()->ok) {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("tm_inspect"), "OK");
  }
  else
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("tm_inspect"), "not OK");
  }
}

void RosPage::change_control_box_io0_button() {
  rclcpp::Client<tm_msgs::srv::SetIO>::SharedPtr client = node->create_client<tm_msgs::srv::SetIO>("set_io");
  ...
  if (!lastStatus) {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "set on");
  }
  else
  {
    request->state = tm_msgs::srv::SetIO::Request::STATE_OFF;
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "set off");
  }
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "Successfully send Ctrl_DO[0]");
}

void RosPage::change_control_box_io15_button() {
  rclcpp::Client<tm_msgs::srv::SetIO>::SharedPtr client = node->create_client<tm_msgs::srv::SetIO>("set_io");
  ...
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("tm_inspect"), "Successfully send Ctrl_DO[15]");
}

RosPage::RosPage(std::string nodeName) : lastStatus(false) {
...
RosPage::~RosPage() {
  rclcpp::shutdown();
  RCLCPP_INFO_STREAM(rclcpp::get_logger("tm_inspect"), "call ~RosPage");
}
