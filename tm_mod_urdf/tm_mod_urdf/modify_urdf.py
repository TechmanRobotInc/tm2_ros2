...
def _gen_urdf(args=None):
    ...
    tm_model = 'reference'
    ###############################################################################################
    # You can simple restore some nominal kinematic parameters by using specific keyword settings
    if len(sys.argv) == 4:
    ...
    for key in model_map:
        if original_model.startswith(key):
            series_name = model_map[key]
            break

    if series_name == '':
        node.get_logger().error(f'unknown model series for: {original_model}')
        return

    curr_path = os.path.dirname(os.path.abspath(__file__))
    dirs = ['tm2_ros2', 'install']
    ind = -1
    for d in dirs:
    ...
    if ind == -1:
        node.get_logger().error('workspace directory not found')
        return

    src_path = curr_path[:ind] + 'tm2_ros2'
    urdf_path = ''
    target_sub_dir = os.path.join('tm_description', 'cobot_s', f'{series_name}_description', 'urdf')

    target_norm = os.path.normpath(target_sub_dir)
    for dirpath, dirnames, filenames in os.walk(src_path):
        if os.path.normpath(dirpath).endswith(target_norm):
            urdf_path = dirpath
            break

    if urdf_path == '':
        node.get_logger().error(f'URDF path not found: {target_sub_dir}')
        return
