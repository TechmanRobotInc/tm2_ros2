...
{
public:
  MinimalSubscriber() : Node("demo_get_svr_response")
  {
...
private:
  ...
  {
    RCLCPP_INFO_STREAM(this->get_logger(), "SvrResponse: id is = " << msg->id << ", mode is " << (int)msg->mode
                                                                   << ", content is " << msg->content
                                                                   << ", error code is " << (int)msg->error_code);
  }
